...
# What's this? There's another group of drivers that comes out of nowhere to join the race!
# Add each element from the others list to the end of the drivers list.
...
drivers.pop(0)  # removes first element from the drivers
...
d1 = drivers.pop(0)  # remove first
drivers.append(d1)  # add to the last position
...
# 7. Pierre
pos = 0
while pos <= len(drivers):
    print(f"{pos+1}. {drivers[pos]}")
    pos += 1
